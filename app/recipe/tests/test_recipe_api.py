...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipes_by_tags(self):
        ...
        """Passing parameters to the GET request"""
        res = self.client.get(
            RECIPE_URL,
        ...
        """
        Serializing the recipes object for checking
        if they are contained inside the response
        """
        serializer1 = RecipeSerializer(recipe1)
        serializer2 = RecipeSerializer(recipe2)

        """
        Serializing the recipe object for checking
        if it is not contained inside the response
        """
